...
export async function getProperties(rooms, property_type, price, location) {
  ...
  if (rooms || property_type || price || location) { //location ?
    sqlStatement += `WHERE `;
  }
  ...
  if (rooms) {
    ...
    if (property_type || price || location) { ////location ?
      sqlStatement += `AND `;
    }
  }
  if (property_type) {
...
if(location){//location ?
  sqlStatement += `location = '${location}' `; ////location ?
  console.log(sqlStatement);
  if (property_type || price || rooms) { ////location ?
    sqlStatement += `AND `;
  }
}
